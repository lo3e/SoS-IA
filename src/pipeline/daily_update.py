...
def update_fixtures(conn):
    ...
    for match_id, date_str in rows:
        ...
        try:
            ...
            if -7 <= delta_days <= DAYS_AHEAD:
                to_update.append(match_id)
        except Exception:
            continue

    logger.info(f"📅 Fixtures da aggiornare (-7 → +{DAYS_AHEAD} giorni): {len(to_update)}")
...
def main():
    ...
    # 3c) evaluation
    if eval_round:
        evaluate_on_round(round_number=eval_round, season=season, model_path=model_path)
    else:
